...
def create_workflows_runs_from_analysis_run(
...
def create_wru_draft(
    workflow: Workflow, libraries: List[Library]
) -> wru.WorkflowRunUpdate:
...
def _map_model_library_to_wru_library(lib: Library) -> wru.Library:
    wru_lib = wru.Library(orcabusId=lib.orcabus_id, libraryId=lib.library_id)
    return wru_lib


def _map_arsc_analysis_run_to_wru_analysis_run(
    run: arsc.AnalysisRunStateChange,
) -> wru.AnalysisRun:
    wru_anr = wru.AnalysisRun(orcabusId=run.orcabusId, name=run.analysisRunName)
    return wru_anr
