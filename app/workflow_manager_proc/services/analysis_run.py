...
from workflow_manager.models.analysis_run import AnalysisRunStates
from workflow_manager_proc.domain.event import ari, arf, arsc
from workflow_manager.models import (
    AnalysisRun,
    Analysis,
    Library, AnalysisContext, AnalysisRunReadset
)
...
def create_analysis_run(event: ari.AnalysisRunInitiated) -> None:
    ...
    """
    logger.info(f"Start processing ARSC: {event}")
    ...
    analysis_run_db: QuerySet = AnalysisRun.objects.query(analysis_run_name=event.analysisRunName)
    if analysis_run_db.exists():
        ...
        analysis_run = AnalysisRun(
            ...
            status=AnalysisRunStates.DRAFT.value,
    ...
    # If there were no issues in the record creation, we emit the new state
    logger.info("Emitting AnalysisRunStateChange event...")
    emit_arsc_from_db_record(db_record=analysis_run)
...
def finalise_analysis_run(event: arf.AnalysisRunFinalised):
    ...
    analysis_run_db: AnalysisRun = AnalysisRun.objects.get(orcabus_id=event.analysisRunId)
    assert analysis_run_db is not None, f"AnalysisRunFinalised: AnalysisRun record does not exist!"
    assert analysis_run_db.status != AnalysisRunStates.READY.value, "Cannot finalise record that is in READY state!"

    # AnalysisRunId: can't be updated, has to match
    assert event.analysisRunId == analysis_run_db.orcabus_id, "AnalysisRun IDs don't match!"
    ...
    # Readsets: are mandatory,, but Readsets can be changed (if not present)
    ...
    for l in event.libraries:
        ...
        assert analysis_run_db.libraries.get(library_id=lid) == analysis_run_db.libraries.get(orcabusId=lod)
        rss = l.readsets
        for rs in rss:
            rs_db = analysis_run_db.analysisrunreadset_set.get(rs.orcabusId, None)
            if rs_db:
    ...
    # emit ARSC event
    analysis_run_db.status = "READY"
    analysis_run_db.save()
    emit_arsc_from_db_record(db_record=analysis_run_db)

    logger.info(f"AnalysisRun finalised.")
...
def emit_arsc_from_db_record(db_record: AnalysisRun) -> None:
    arsc_event = map_analysis_run_to_arsc(analysis_run=db_record)
    logger.info("Emitting ARSC.")
    emit_event(event_type=EventType.ARSC, event_json=arsc_event.model_dump_json())
