...
def finalise_analysis_run(event: arf.AnalysisRunFinalised):
    ...
    assert analysis_run_db.get_latest_state().status == Status.DRAFT.value, "Cannot finalise record that is no in DRAFT state!"
...
def get_arsc_hash(arsc: arsc.AnalysisRunStateChange) -> str:
    ...
    keywords.append(arsc.timestamp)
