...
class WorkflowRunActionViewSet(ViewSet):
    ...
    @extend_schema(
        responses=AllowedRerunWorkflowSerializer, description="Allowed rerun workflows"
    )
    @action(
        detail=True,
        methods=["get"],
        url_name="validate_rerun_workflows",
        url_path="validate_rerun_workflows",
    )
    def validate_rerun_workflows(self, request, *args, **kwargs):
        wfl_run = get_object_or_404(self.queryset, pk=kwargs.get("pk"))
        is_valid = wfl_run.workflow.name in [
            member.value for member in AllowedRerunWorkflow
        ]
        ...
        if wfl_name == AllowedRerunWorkflow.RNASUM.value:
            allowed_dataset_choice = RERUN_INPUT_SERIALIZERS[
                wfl_name
            ].allowed_dataset_choice
    ...
    @action(detail=True, methods=["post"], url_name="rerun", url_path="rerun")
    def rerun(self, request, *args, **kwargs):
        ...
        if wfl_run.workflow.name not in [
            member.value for member in AllowedRerunWorkflow
        ]:
            return Response(
                f"This workflow type is not allowed: {wfl_run.workflow.name}",
        ...
        if is_wfr_deprecated:
            return Response(
                {
                    "detail": "Workflow run has been deprecated and rerun is not allowed."
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            logger.exception(
                "Failed to create rerun audit comment for workflow run %s, user email: %s, new portal run id: %s, error: %s",
                wfl_run.orcabus_id,
                user_email,
                new_portal_run_id,
                e,
...
def construct_rerun_eb_detail(wfl_run: WorkflowRun, input_body: dict) -> dict:
    ...
    return {
        ...
        "libraries": LibrarySerializer(
            wfl_run.libraries.all(), many=True, camel_case_data=True
        ).data,
        "payload": new_payload,
...
def construct_rnasum_rerun_payload(wfl_run: WorkflowRun, input_body: dict) -> dict:
    ...
    if not allow_rerun_duplication:
        ...
        wfl_runs = WorkflowRun.objects.annotate(
            library_ids=StringAgg(
                "libraries__library_id",
                delimiter=",",
                ordering=F("libraries__library_id").asc(),
            )
        ).filter(workflow=wfl_run.workflow, library_ids=library_ids_string)
        past_dataset = set()
        for run in wfl_runs:
            try:
        ...
        if input_body["dataset"] in past_dataset:
            raise RerunDuplicationError(
                f"Dataset '{input_body['dataset']}' has been run in the past. "
