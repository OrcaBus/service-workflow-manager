...
class TestData:
    """ImplsNote: the class implemented fluent API. Chain each method calls to create the test fixture combo.

    FIXME to also simulate AnalysisRunReadset and AnalysisRunState
    ...
    def __init__(self):
        ...
        # Keeping track for analysis runs created
    ...
    def _setup_requirements():
        ...
        #     name="clinical",
        #     usecase="approval",  FIXME approval context
        ...
        # wgs_clinical_analysis.contexts.add(clinical_context)  FIXME approval context
        wgs_clinical_analysis.contexts.add(clinical_compute_context)
        ...
        cttso_analysis.save()
        # cttso_analysis.contexts.add(clinical_context)  FIXME approval context
        cttso_analysis.contexts.add(clinical_compute_context)
    ...
    def assign_analysis(self):
        # Handle QC analysis
        self.create_qc_analysis()

        # Handle WGS analysis
        self.create_wgs_analysis()

        # Handle ctTSO analysis
        self.create_cttso_analysis()
    ...
    def create_qc_analysis(self):
        ...
        temp_storage_context_rnx, _ = RunContext.objects.get_or_create(
        ...
        for lib in self.libraries:
            ...
            if lib['type'] in ['WGS', 'WTS']:
                ...
                analysis_run.contexts.add(research_compute_context_rnx)
                analysis_run.contexts.add(temp_storage_context_rnx)
    ...
    def create_wgs_analysis(self):
        ...
        for sbj in pairing:
            if pairing[sbj]['tumor'] and pairing[sbj]['normal']:
                ...
                storage_context_rnx, _ = RunContext.objects.get_or_create(
                ...
                ar_wgs.contexts.add(compute_context_rnx)
                ar_wgs.contexts.add(storage_context_rnx)
    ...
    def create_cttso_analysis(self):
        ...
        for lib in self.libraries:
            ...
            if lib['type'] in ['ctDNA'] and lib['assay'] in ['ctTSOv2']:
                ...
                analysis_run.save()
                analysis_run.libraries.add(lib_record)
                analysis_run.contexts.add(compute_context_rnx)
                analysis_run.contexts.add(storage_context_rnx)
                self.analysis_runs.append(analysis_run)

        return self

    def prep_workflow_runs(self):
        ...
        for valid_arun in valid_aruns:
            ...
            # create WorkflowRuns
            self._create_workflowrun_for_analysis(valid_arun)
    ...
    @staticmethod
    def _create_workflowrun_for_analysis(analysis_run: AnalysisRun):
