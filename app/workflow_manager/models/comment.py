...
class CommentSeverity(models.TextChoices):
    ...
    Refer issue: https://github.com/OrcaBus/service-workflow-manager/issues/144 for more details.
    """
...
class Comment(OrcaBusBaseModel):
    ...
    severity = models.CharField(
        max_length=16, choices=CommentSeverity.choices, default=CommentSeverity.INFO
    )  # severity of the comment
    ...
    def clean(self):
        ...
        if has_workflow_run == has_analysis_run:
            raise ValidationError(
                "A comment must be linked to exactly one of workflow_run or analysis_run."
            )
